"""
This script is used to build stat graph for the database2
RUN: python3 build_graph_for_database2.py
"""
...
# 读取 CSV 文件
failure_analysis = pd.read_csv('../database2/failure_stat.csv')

# 八种漏洞的文件路径
vulnerability_files = [
    '../database2/5UnsecuredBalance_analysis.csv', '../database2/6MisuseOfOrigin_analysis.csv',
    '../database2/3FailedSend_analysis.csv', '../database2/4TimestampDependence_analysis.csv',
    '../database2/7Suicidal_analysis.csv', '../database2/1Reentrancy_analysis.csv',
    '../database2/2UncheckedCall_analysis.csv', '../database2/8Securify-Reentrancy_analysis.csv'
]
...
# 统计 success = True 的情况
success_true_tx = 205079 - len(failure_analysis[failure_analysis['TxSpector_False'].notna()])
success_true_framework = 205080 - len(failure_analysis[failure_analysis['Framework_False'].notna()])
...
# ax1.set_title('Database 2 - Comparison of Vulnerability Detection by TxSpector and Framework', fontsize=16,
#               fontweight='bold')
ax1.set_xticks(x)
ax1.set_xticklabels(labels, rotation=0, fontsize=9, fontweight='bold')  # 调整x轴标签字体大小
ax1.legend(fontsize=12)

# 绘制饼状图（success 的情况）
success_counts_tx = [success_true_tx, 205080 - success_true_tx]
success_counts_framework = [success_true_framework, 205080 - success_true_framework]
success_colors = ['#a1c4fd', 'white']  # 使用淡雅的颜色
success_hatches = ['///', '']
...
ax2.set_title('TxSpector Success', fontsize=14, fontweight='bold')
...
for i, patch in enumerate(wedges):
    if success_hatches[i]:
        patch.set_hatch(success_hatches[i])
ax3.set_title('Framework Success', fontsize=14, fontweight='bold')
...
add_pie_labels(ax2, success_counts_tx, 205080)
add_pie_labels(ax3, success_counts_framework, 205080)
...
# 保存为PDF文件
plt.savefig('../database2/database2_result.pdf', format='pdf')
